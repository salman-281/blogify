...
import { BlogPost, BusinessData, LifestyleData } from '@/lib/blogs'
...
const page = () => {
  ...
  const [lifestyle, setLifestyle] = useState<BlogPost[]>([]);

 useEffect(() => {
  const lifestyleDataFun = async () => {
    let data = await LifestyleData;
    setLifestyle(data);
  }
  lifestyleDataFun();
}, []);
  ...
  <h2 className="text-lg font-bold oswald tracking-tighter sm:text-2xl">Lifestyle Picks</h2>
  <p className="text-sm dm_sans text-gray-500 text-center">
    Curated tips, trends, and routines to elevate your daily life.
  </p>
  ...
  <p className="text-sm dm_sans text-gray-500 text-center max-w-xl">
    Unlock mindful living, wellness hacks, and productivity tips designed to help you create a more balanced and meaningful lifestyle.
  </p>
</div>
        </div>
