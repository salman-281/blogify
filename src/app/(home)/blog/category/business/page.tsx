...
import { BlogPost, BusinessData } from '@/lib/blogs'
...
const page = () => {
  ...
  const [business, setbusiness] = useState<BlogPost[]>([])

 useEffect(() => {
  const bussinessDatafun = async () => {
    let data = await BusinessData; // remove the parentheses
    setbusiness(data);
  }
  bussinessDatafun();
}, []);
  ...
  <h2 className="text-lg font-bold oswald tracking-tighter sm:text-2xl">Business Pulse</h2>
  <p className="text-sm dm_sans text-gray-500 text-center">
    Unlock strategies, insights, and stories from the world of commerce.
  </p>
  <h3 className="text-md font-semibold oswald text-center sm:text-lg">
    Build Smart, Lead Bold
  </h3>
  <p className="text-sm dm_sans text-gray-500 text-center max-w-xl">
    Discover entrepreneurial wisdom, market trends, and success stories that empower you to innovate and grow in today’s economy.
  </p>
</div>
        </div>
        <div className="mx-auto grid max-w-7xl items-start gap-6 py-5 lg:grid-cols-3 xl:grid-cols-4 md:grid-cols-2 sm:grid-cols-2">
          {
          business.map((post: BlogPost, i: number) => (
