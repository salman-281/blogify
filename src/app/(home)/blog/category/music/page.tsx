...
import { BlogPost, BusinessData, MusicData } from '@/lib/blogs'
...
const page = () => {
  ...
  const [music, setMusic] = useState<BlogPost[]>([]);

  useEffect(() => {
    const musicDataFun = async () => {
      let data = await MusicData;
      setMusic(data);
    }
    musicDataFun();
  }, []);
  ...
  <h2 className="text-lg font-bold oswald tracking-tighter sm:text-2xl">Music Vibes</h2>
  <p className="text-sm dm_sans text-gray-500 text-center">
    Dive into the rhythm of sound, stories, and soulful expression.
  </p>
  ...
  <p className="text-sm dm_sans text-gray-500 text-center max-w-xl">
    From artist spotlights to genre deep-dives, explore how music shapes emotion, culture, and creativity across the globe.
  </p>
</div>
        </div>
