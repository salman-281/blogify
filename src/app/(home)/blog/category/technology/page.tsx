...
import { BlogPost, TechnologyData } from '@/lib/blogs'
...
const page = () => {
  ...
  const [technology, setTechnology] = React.useState<BlogPost[]>([]);
  
    useEffect(() => {
      const technologyDataFun = async () => {
        let data = await TechnologyData;
        setTechnology(data);
      }
      technologyDataFun();
    }, []);
        ...
        <div className="container ">
          <div className="flex flex-col items-center justify-center  text-center">
            <div className="flex justify-center flex-col items-center p-2 gap-4 w-full">
              <h2 className="text-lg font-bold oswald tracking-tighter sm:text-2xl">Technology Insights</h2>
              <p className="text-sm dm_sans text-gray-500 text-center">
                Dive into the world of innovation and tech trends shaping our future.
              </p>
              <h3 className="text-md font-semibold oswald text-center sm:text-lg">
                Smarter Tech, Smarter Life
              </h3>
              <p className="text-sm dm_sans text-gray-500 text-center max-w-xl">
                From AI breakthroughs to gadget reviews, explore how technology is transforming everyday life and powering the digital age.
              </p>
            </div>
          </div>
          <div className="mx-auto grid max-w-7xl items-start gap-6 py-5 lg:grid-cols-3 xl:grid-cols-4 md:grid-cols-2 sm:grid-cols-2">
            {technology.map((post, i) => (
