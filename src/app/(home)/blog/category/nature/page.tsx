...
import { BlogPost, BusinessData, NatureData } from '@/lib/blogs'
...
const page = () => {
  ...
  const [nature, setNature] = useState<BlogPost[]>([]);

  useEffect(() => {
    const natureDataFun = async () => {
      let data = await NatureData;
      setNature(data);
    }
    natureDataFun();
  }, []);
  ...
  <h2 className="text-lg font-bold oswald tracking-tighter sm:text-2xl">Nature Chronicles</h2>
  <p className="text-sm dm_sans text-gray-500 text-center">
    Discover the untold stories and serene beauty of the natural world.
  </p>
  <h3 className="text-md font-semibold oswald text-center sm:text-lg">
    Breathe In, Explore Out
  </h3>
  <p className="text-sm dm_sans text-gray-500 text-center max-w-xl">
    Journey through the wild with features on landscapes, ecosystems, and sustainability. Learn why protecting our planet matters now more than ever.
  </p>
</div>
